...
export default function Shijing(props: { data: any, menus: any }) {
    ...
    <>
      ...
      <Grid>
        {data && data.map((result: any, index: number) => (
          (<Grid
            item
            key={`${result['title']}-${result['section']}-${result['section']}-${index}`}
            xs={6}
            sm={6}
            md={4}
          >
            ...
            >
              <CardContent>
                <Button variant="outlined" id={result['title']} sx={{ mr: 2 }}>
                  {result['title']} - {result['chapter']} - {result['section']}
                </Button>
              ...
              <CardActions>
                <List dense={true} disablePadding={true}>
                  {result['content'] && result['content'].map((text: any, zindex: number) => (
